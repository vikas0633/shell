...
SCRIPT_NAME="20130324_illumina_SE_afterTrimming_BWA.sh"
...
do 
nice -n 19 bwa aln -t $nthreads -l 28 "$GENOME" $file > $file.sai;
echo $file "has been aligned on" `date "+20%y%m%d_%H%M"` >> $LOGFILE
nice -n 19 bwa samse -n 30 "$GENOME" $file.sai $file | nice -n 19 samtools view -bt "$GENOME".fai -| nice -n 19 samtools sort - "$file"_sorted
