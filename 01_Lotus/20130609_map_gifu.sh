...
#for file in *.fastq;
#do 
nice -n 19 bwa aln -t $threads -l 28 "$ref_file" $read_1_1 > $read_1_1.sai;
nice -n 19 bwa aln -t $threads -l 28 "$ref_file" $read_1_2 > $read_1_2.sai;
#done;

rg="sym10_R1.fastq"
rg="@RG\tID:$rg\tSM:$rg\tPL:illumina\tLB:lib1\tPU:unit"
####--------------mapping the reads ---------------------------------####
nice -n 19 bwa sampe -r $rg -P "$ref_file" "$read_1_1"".sai" "$read_1_2"".sai"  "$read_1_1" "$read_1_2" | nice -n 19 samtools view -bt "$ref_file".fai -| nice -n 19 samtools sort - "$read_1_1"_sorted
