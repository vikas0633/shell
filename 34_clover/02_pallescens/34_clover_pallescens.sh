...
####################################
### RUNNING Blastp - on Genome #####
####################################
cd /home/vgupta/LotusGenome/01_vgupta/12_pallescens/final_fasta
source /com/extra/BLAST/2.2.26/load.sh
source /com/extra/BLASTALL/2.2.26/load.sh
QUERY="pallescens.protein.fa"
DATABASE="/home/vgupta/80_blastDatabases/nr.plant"
FILE_DIR="/home/vgupta/LotusGenome/01_vgupta/12_pallescens/final_fasta"
BLAST_OUT="/home/vgupta/LotusGenome/01_vgupta/12_pallescens/final_fasta/01_blastout"
mkdir -p $BLAST_OUT
nice -n 19 formatdb -i $DATABASE -p T -o T
nice -n 19 perl ~/script/perl/fasta_splitter.pl -n-parts-sequence 100 $QUERY
qx -v --nodes=10 --dir=. -i "*.part-*.fa" -o "*.txt" -i 20140902_blast.sh dispatch -r 20140902_blast.sh > jobscript.sh
for f in *.part-*.fa; do echo "nice -n 19 blastp  -max_target_seqs=20 -num_threads=8 -db $DATABASE -query $f -outfmt 6 -out $f.txt"; done > 20140902_blast.sh 
sbatch jobscript.sh
mv pallescens.protein.part-*.fa.txt 01_blastout/
