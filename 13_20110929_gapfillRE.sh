...
pair_read_1=$pair_read_1.filtered
pair_read_2=$pair_read_2.filtered
pair_read_3=$pair_read_3.filtered
pair_read_4=$pair_read_4.filtered
pair_read_5=$pair_read_5.filtered
pair_read_6=$pair_read_6.filtered
pair_read_7=$pair_read_7.filtered
pair_read_8=$pair_read_8.filtered
...
### Take out all the N region positions and print to a file
nice -n 19 python 13c.py $ref_file > $ref_file.N_regions.txt_im
### get the chromosome

input=$ref_file.N_regions.txt_im
...
echo END >>"$input"direct_rep.$x
### for reverse strand
cat $pair_read_1.unmapped.fq.unique_all_rep_aln.sam| nice -n 19 perl -lane 'print $_ if ($F[1] == 16)'|sort|cut -f 1,3,4  >check.txt
echo END >>check.txt
strand=16
nice -n 19 python 13i.py check.txt out.txt $rep_file $x $strand
nice -n 19 python 13j.py out.txt rep_region >"$input"reverse_rep.$x
echo END >>"$input"reverse_rep.$x

###find best orientation 
python 13l.py "$input"direct_rep.$x "$input"reverse_rep.$x > "$dat"_summary.txt.$x
...
done <$ref_file.N_regions.txt_im ### for input file


### summarizing output
nice -n 19 python 13k.py rep_pos "$dat"_summary.txt.1 "$dat"_summary.txt.2 "$dat"_summary.txt.3 >"$input"summary_"$dat"
